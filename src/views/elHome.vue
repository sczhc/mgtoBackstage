                        ...
                        <el-collapse v-model="activeCollapseName">
                            <el-collapse-item name="dargFile">
                                <div slot="title" @click="stopEvent">
                                    <el-button class="btn btn-lineheight" @click="startUpload">開始上傳</el-button>
                                ...
                                <darg-file ref="darg-file" :presetLang="form.indicator" :newLang="language" :imgList="imgList" @checked="checkedList" :afferentList="afferentList"></darg-file>
                            </el-collapse-item>
                        </el-collapse>
                    </div>
                </el-form-item>
            ...
            <el-col :span="24">
                <div class="form-actions">
                    <template v-if="types === 'create'">
                        <el-button type="primary" class="btn btn-primary" @click="onSubmit">創建</el-button>
                    </template>
                    <template v-else>
                        <el-button type="primary" class="btn btn-primary" @click="onSubmit">更新</el-button>
                    </template>
                    <a class="btn btn-back" href="/admintools/news_draft/">返回</a>
                    <template v-if="types !== 'create'">
                        <el-button class="btn btn-preview" @click="handlePreview">預覽</el-button>
                    </template>

                    <template v-if="types === 'update'">
                        <el-button class="btn btn-approval">提交審批</el-button>
                        <el-button class="btn btn-release">提交直接發佈</el-button>
                    </template>
                </div>
            </el-col>
        </el-row>
        ...
        <div class="label" v-if="treeResult">
            <div class="tree-label">
                ...
                <div class="tree-button">
                    <el-button class="btn" @click="submitClick">確定</el-button>
                    <el-button class="btn" @click="treeResult = false">Add Tag</el-button>
                </div>
            </div>
        </div>
            ...
            <div class="addTag-content">
                <el-form ref="addTagForm" :label-width="resizeWidth >= 768 ? '150px' : ''">
                    ...
                    <div class="form-action">
                        <el-button class="btn btn-primary">添加</el-button>
                    </div>
                </el-form>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        complexity() {
            let num
            let result = this.form.multilingualism.some((item, index) => {
                num = index
                return item.language == 'zh_CN'
            })
            if (result) {
                let content = this.form.multilingualism[num].content
                let existence = this.isNull(content)
                if (existence) {
                    this.$confirm('簡體已經有內容，確定要覆蓋?', {
                        confirmButtonClass: 'confirm-button',
                        cancelButtonClass: 'cancel-button'
                    }).then(() => {
                        let arr = Object.values(this.form.multilingualism[0].content)
                        this.form.multilingualism[num].content = {
                            title: this.traditionalized(arr[0]),
                            content: this.traditionalized(arr[1]),
                            remark: this.traditionalized(arr[2]),
                        }
                    }).catch()
                } else {
                    let arr = Object.values(this.form.multilingualism[0].content)
